...
def get_incidents_api(incident_id):
    try:
        ...
        if incident_id is not None and type(incident_id) is int:
            res = incidents.Incident(id=incident_id).get_json(deleted=get_deleted, field_lists=fields)
            return jsonify(res)
        else:
            res = incidents.Incident().get_json(lim=limit, index=fr, deleted=get_deleted, field_lists=fields)
            return jsonify(res)
